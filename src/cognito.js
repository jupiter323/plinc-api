const { config, CognitoIdentityServiceProvider, CognitoIdentityCredentials, AmazonCognitoIdentity } = require('aws-sdk');
...
const { AWS_REGION, USER_POOL_ID:
        UserPoolId,
        IDENTITY_POOL_ID: IdentityPoolId,
        CLIENT_ID: ClientId } = process.env;
...
const createUser = ({ username: Username, adminPassword: TemporaryPassword, userAttributes: UserAttributes }) => (
...
const federatedAccess = token => new Promise((resolve, reject) => {
...
const generateUser = async () => {
  ...
  const { AuthenticationResult: { IdToken } } = data;
